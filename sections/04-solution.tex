\section{实现方案}
\subsection{作品简介}
本方案基于现有\BoardModel{}设计\RuntimeName{}。主机端完成模型训练、量化及执行计划生成；板端通过\ProcessorCore{}、\VectorUnit{}和\AcceleratorName{}三种后端执行推理，输出带样本标识的结果与逐层记录。首个验证对象为\AIModel{}，后续扩展视觉与智能感知，软硬件按下述路径研发和验证。

\subsection{技术方案}
\subsubsection{系统分层与数据流}
\begin{figure}[H]
\centering
\input{assets/diagrams/software-architecture.tex}
\caption{\SoftwareArchitectureCaption}
\label{fig:software}
\end{figure}
...
\begin{figure}[H]
\centering
\input{assets/diagrams/hardware-architecture.tex}
\caption{\HardwareArchitectureCaption}
\label{fig:hardware}
\end{figure}
